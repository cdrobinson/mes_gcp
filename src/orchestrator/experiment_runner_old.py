...
class ExperimentRunner:
    ...
    def __init__(self, config_path: str):
        ...
        # Initialize available metrics (simplified - direct imports)
    ...
    def _discover_metrics(self) -> Dict[str, BaseMetric]:
        """Dynamically discover available metric classes"""
        metrics = {}
        
        # Import metric modules and discover BaseMetric subclasses
        metric_modules = [
            'src.metrics.safety.safety',
            'src.metrics.safety.privacy', 
            'src.metrics.transcription.transcript_quality',
            'src.metrics.summarisation.gptscore',
            'src.metrics.summarisation.qafacteval',
            'src.metrics.summarisation.chainpoll'
        ]
        ...
        for module_name in metric_modules:
            try:
                ...
                # Find all classes that inherit from BaseMetric
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    if (obj != BaseMetric and 
                        issubclass(obj, BaseMetric) and 
                        hasattr(obj, '__init__')):
                        
                        # Create instance with minimal args for discovery
                        try:
                            if 'Safety' in name:
                                metric_instance = obj()
                            elif 'Privacy' in name:
                                metric_instance = obj(project_id=self.config['project'])
                            elif 'TranscriptQuality' in name:
                                metric_instance = obj()
                            else:
                                # Metrics that need judge client will be initialized later
                                metric_instance = obj(judge_client=None)
                            
                            metrics[metric_instance.name] = metric_instance
                            logger.info(f"Discovered metric: {metric_instance.name}")
                            
                        except Exception as e:
                            logger.warning(f"Could not initialize metric {name}: {e}")
                            
            except ImportError as e:
                logger.warning(f"Could not import metric module {module_name}: {e}")
    ...
    def _run_single_experiment(self, experiment: Dict[str, Any], audio_files: List[str]):
        ...
        # Initialize Gemini client for this experiment
        ...
        experiment_metrics = self._init_experiment_metrics(experiment, gemini_client)
    ...
    def _init_experiment_metrics(self, experiment: Dict[str, Any], gemini_client: GeminiClient) -> List[BaseMetric]:
        """Initialize metrics for a specific experiment"""
        ...
        for metric_name in metric_names:
            if metric_name in self.available_metrics:
                ...
                # For metrics that need a judge client, create new instance
                if hasattr(metric, 'judge_client') and metric.judge_client is None:
                    # Create new instance with judge client
                    metric_class = type(metric)
                    if 'bias' in metric_name:
                        from ..metrics.safety.safety import BiasMetric
                        metric = BiasMetric(judge_client=gemini_client)
                    elif 'gptscore' in metric_name:
                        from ..metrics.summarisation.gptscore import GPTScoreMetric
                        metric = GPTScoreMetric(judge_client=gemini_client)
                    elif 'qafacteval' in metric_name:
                        from ..metrics.summarisation.qafacteval import QAFactEvalMetric
                        metric = QAFactEvalMetric(judge_client=gemini_client)
                    elif 'chainpoll' in metric_name:
                        from ..metrics.summarisation.chainpoll import ChainPollMetric
                        metric = ChainPollMetric(judge_client=gemini_client)
    ...
    def get_experiment_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "use_cases": list(df['use_case'].unique()),
            "avg_processing_time": df['processing_time'].mean(),
            "total_processing_time": df['processing_time'].sum()
        }
        
        # Add metric summaries
        metric_columns = [col for col in df.columns if not col.startswith(('experiment_', 'model_', 'use_case', 'audio_', 'response_', 'processing_', 'timestamp'))]
