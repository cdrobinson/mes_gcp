...
class AbstractSummarizationMetric(BaseMetric):
    """Abstract base class for summarization metrics."""
    @abstractmethod
    def calculate(self, summary: str, **kwargs) -> Dict[str, Any]: # type: ignore
        """
        Calculates summarization-specific metrics.
        Args:
...
class SummaryLength(AbstractSummarizationMetric):
...
class SummarySentenceCount(AbstractSummarizationMetric):
    """Calculates the number of sentences in the summary."""
    def calculate(self, summary: str, **kwargs) -> Dict[str, Any]:
        if not summary or not summary.strip():
            return {"summary_sentence_count": 0}
        # Simple sentence split based on common delimiters.
        # For more accuracy, a proper NLP library would be needed, but keeping it basic.
        delimiters = ".!?"
        count = 0
        for char_idx, char in enumerate(summary):
            if char in delimiters:
                # Ensure it's not part of a sequence (e.g., "...") or at the very end of a trailing whitespace
                if char_idx + 1 < len(summary) and summary[char_idx+1].isspace():
                    count +=1
                elif char_idx == len(summary) -1: # Ends with a delimiter
                    count +=1
        if count == 0 and len(summary.strip()) > 0 : # If no delimiters but text exists, assume one sentence
             count = 1
        return {"summary_sentence_count": count}

class AverageSentenceLength(AbstractSummarizationMetric):
...
class SummaryRepetitiveness(AbstractSummarizationMetric):
...
class HedgingLanguageCount(AbstractSummarizationMetric):
    ...
    def calculate(self, summary: str, **kwargs) -> Dict[str, Any]:
        ...
        count = 0
        # Simple check for individual words; for phrases, more complex matching would be needed
        # For this basic version, we'll count occurrences of single words from the list.
        # A more robust solution would involve checking for multi-word phrases.
...
class FleschReadingEaseScore(AbstractSummarizationMetric):
